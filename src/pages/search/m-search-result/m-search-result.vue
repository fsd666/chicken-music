 ...
 * Description: 搜索结果
 * -----
 * Last Modified: Tuesday, 12th November 2019 2:23:56 pm
 * Modified By: Fan
 * -----
...
<template>
    <div id="m-search-result">
        ...
        >
            <ul class="list">
                <div class="list-item" v-if="zhida.type === 2" @click="toSingerDetail">
                    <m-icon type="user" :size="12" color="hsla(0,0%,100%,.3)"></m-icon>
                    <span class="list-item-song">{{zhida.Fsinger_name}}</span>
                </div>
                <li
                    class="list-item"
                ...
                >
                    ...
                    <span class="list-item-song">{{item.songname}}-{{singerFormat(item.singer)}}</span>
                </li>
                <m-loading></m-loading>
            </ul>
        </m-scroll>
...
export default {
    ...
    methods: {
        search () {
            getSearchResult(this.query, this.pageSize, 1, this.perpage).then(res => {
                this.zhida = {
                    Fsinger_mid: res.data.zhida.singermid,
                    Fsinger_name: res.data.zhida.singername,
                    type: res.data.zhida.type
                };
        ...
        toSingerDetail () {
            this.setSinger(this.zhida);
            this.$router.push(`/search/${this.zhida.Fsinger_mid}`);
        },
        ...mapMutations(["saveSearchHistory", "setSinger"])
    },
    computed: {
        ...mapGetters({
