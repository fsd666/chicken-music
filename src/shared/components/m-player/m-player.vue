 ...
 * Description: 音乐播放组件
 * -----
 * Last Modified: Wednesday, 13th November 2019 9:55:23 am
 * Modified By: Fan
 * -----
    ...
    <div id="m-player" v-if="musicList.length>0">
                ...
                <div class="bottom">
                    ...
                    <div class="play-wrapper">
                        ...
                        <m-icon
                            ...
                            v-if="isFavorite(musicList[musicIndex].songmid)"
                        ></m-icon>
                        <m-icon
                            type="favorite"
                            :size="34"
        ...
        <div class="m-player-mini" v-show="!fullScreen">
            ...
            <div class="song-msg">
                ...
                <span class="song-msg-singer">{{singer}}</span>
            </div>
            <m-icon
                class="mini-icon"
                type="play"
...
import { getVKey, getLyric } from "../../../api/music";
...
export default {
    ...
    methods: {
        ...
        lyricFormatter (res) {
            ...
            if (res.lyric) {
                let lyric = CryptoJS.enc.Base64.parse(res.lyric).toString(CryptoJS.enc.Utf8);
    ...
    watch: {
        musicInfo (newMusicInfo) {
            ...
            if (currentMusic.songmid === this.currentSongMid) {
                return;
            }
            this.currentSongMid = currentMusic.songmid;
            this.addPlayHistory(currentMusic);
            this.lyricArr = [];
            this.lyricIndex = 0;
            this.currentTime = 0;
            this.album = `https://y.gtimg.cn/music/photo_new/T002R300x300M000${currentMusic.albummid}.jpg?max_age=2592000`;
            this.singer = currentMusic.singer.map(item => item.name).join("/");
            Promise.all([
                getVKey(currentMusic.songmid),
                getLyric(currentMusic.songmid)
            ]).then(res => {
                let { vkey, filename } = res[0].data.items[0];
                this.musicFile = `http://dl.stream.qqmusic.qq.com/${filename}?vkey=${vkey}&guid=3655047200&fromtag=66`;
                this.lyricFormatter(res[1]);
            });
        },
        lyricIndex (newIndex) {
            const currentLyricDom = this.$refs.lyricText[newIndex];
            const lyricWrapperDom = this.$refs.lyricWrapper;
            const offsetY = -(lyricWrapperDom.clientHeight / 2);
            if (currentLyricDom.offsetTop > lyricWrapperDom.clientHeight / 2) {
