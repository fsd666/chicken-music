 ...
 * Description: 本地缓存工具
 * -----
 * Last Modified: Tuesday, 12th November 2019 2:40:41 pm
 * Modified By: Fan
 * -----
...
export function removeFavorite (value) {
    let favoriteList = queryFavoriteList();
    const cancelIndex = favoriteList.findIndex(item => item.songmid === value.songmid);
...
export function addPlayHistory (value) {
    ...
    const index = history.findIndex(item => item.songmid === value.songmid);
    if (index === -1) {
        history.unshift(value);
